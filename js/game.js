...
function resize(){W=innerWidth;H=innerHeight;dpr=Math.min(devicePixelRatio||1,(innerWidth<700?1.25:2));canvas.width=W*dpr;canvas.height=H*dpr;canvas.style.width=W+'px';canvas.style.height=H+'px';ctx.setTransform(dpr,0,0,dpr,0,0);if(player){player.x=Math.max(28,Math.min(WORLD_W-28,player.x));player.y=Math.max(28,Math.min(WORLD_H-28,player.y));camera.x=Math.max(0,Math.min(WORLD_W-W,player.x-W/2));camera.y=Math.max(0,Math.min(WORLD_H-H,player.y-H/2))}}addEventListener('resize',resize);resize();function showToast(msg){toast.textContent=msg;toast.classList.add('show');clearTimeout(toastTimer);toastTimer=setTimeout(()=>toast.classList.remove('show'),1550)}
...
let v456FrameLast=0;function loop(now){if(!running)return;if(isIos&&now-v456FrameLast<31){requestAnimationFrame(loop);return}const dt=Math.min(.04,(now-last)/1000||0);last=now;v456FrameLast=now;if(!paused)update(dt);draw();requestAnimationFrame(loop)}
