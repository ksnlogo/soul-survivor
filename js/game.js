...
function resize(){W=innerWidth;H=innerHeight;dpr=Math.min(devicePixelRatio||1,(innerWidth<700?1.5:2));canvas.width=W*dpr;canvas.height=H*dpr;canvas.style.width=W+'px';canvas.style.height=H+'px';ctx.setTransform(dpr,0,0,dpr,0,0);if(player){player.x=Math.max(28,Math.min(WORLD_W-28,player.x));player.y=Math.max(28,Math.min(WORLD_H-28,player.y));camera.x=Math.max(0,Math.min(WORLD_W-W,player.x-W/2));camera.y=Math.max(0,Math.min(WORLD_H-H,player.y-H/2))}}addEventListener('resize',resize);resize();function showToast(msg){toast.textContent=msg;toast.classList.add('show');clearTimeout(toastTimer);toastTimer=setTimeout(()=>toast.classList.remove('show'),1550)}
