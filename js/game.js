...
const stars=Array.from({length:70},()=>({x:Math.random()*WORLD_W,y:Math.random()*WORLD_H,r:Math.random()*1.4+.25,a:Math.random()*.18+.03}));const ruins=Array.from({length:40},()=>({x:rand(140,WORLD_W-140),y:rand(140,WORLD_H-140),r:rand(24,58),rot:rand(0,Math.PI)}));
...
function generateWorld(){selectMapTheme();terrainPatches=Array.from({length:32},(_,i)=>({x:rand(120,WORLD_W-120),y:rand(120,WORLD_H-120),rx:rand(120,330),ry:rand(90,250),rot:rand(0,Math.PI),kind:i%4}));groundDecos=Array.from({length:170},()=>({x:rand(60,WORLD_W-60),y:rand(60,WORLD_H-60),kind:Math.floor(rand(0,5)),rot:rand(0,Math.PI*2),scale:rand(.7,1.45)}));fogWisps=Array.from({length:14},()=>({x:rand(0,WORLD_W),y:rand(0,WORLD_H),r:rand(100,240),speed:rand(4,12),phase:rand(0,Math.PI*2)}));mapPaths=[];for(let p=0;p<2;p++){const horizontal=p!==1,pts=[];if(horizontal){pts.push({x:80,y:rand(WORLD_H*.18,WORLD_H*.82)});pts.push({x:WORLD_W*.32,y:rand(WORLD_H*.15,WORLD_H*.85)});pts.push({x:WORLD_W*.68,y:rand(WORLD_H*.15,WORLD_H*.85)});pts.push({x:WORLD_W-80,y:rand(WORLD_H*.18,WORLD_H*.82)})}else{pts.push({x:rand(WORLD_W*.2,WORLD_W*.8),y:80});pts.push({x:rand(WORLD_W*.15,WORLD_W*.85),y:WORLD_H*.34});pts.push({x:rand(WORLD_W*.15,WORLD_W*.85),y:WORLD_H*.68});pts.push({x:rand(WORLD_W*.2,WORLD_W*.8),y:WORLD_H-80})}mapPaths.push(pts)}mapLandmarks=[];const lmKinds=currentMap.id==='forest'?['shrine','tree','crystal','gate']:currentMap.id==='frost'?['crystal','obelisk','gate','shrine']:currentMap.id==='ember'?['forge','obelisk','crystal','gate']:['obelisk','shrine','gate','crystal'];for(let i=0;i<8;i++){const p=safeWorldPoint(360);mapLandmarks.push({x:p.x,y:p.y,kind:pick(lmKinds),r:rand(32,55),phase:rand(0,Math.PI*2)})}chests=[];for(let i=0;i<8;i++){const p=safeWorldPoint(280);chests.push({x:p.x,y:p.y,opened:false,elite:i>=6})}springs=[];for(let i=0;i<4;i++){const p=safeWorldPoint(300);springs.push({x:p.x,y:p.y,cooldown:0,maxCooldown:34})}obstacles=[];const poi=[...chests.map(o=>[o.x,o.y,105]),...springs.map(o=>[o.x,o.y,125]),...mapLandmarks.map(o=>[o.x,o.y,o.r+70]),[WORLD_W/2,WORLD_H/2,330]],types=currentMap.id==='frost'?['rock','pillar','pillar','ruin','ruin','tree']:currentMap.id==='ember'?['rock','rock','rock','ruin','pillar','tree']:currentMap.id==='crypt'?['pillar','ruin','ruin','tree','rock','pillar']:['rock','rock','tree','tree','pillar','ruin'];for(let n=0;n<130&&obstacles.length<42;n++){const x=rand(110,WORLD_W-110),y=rand(110,WORLD_H-110),type=pick(types),r=type==='rock'?rand(28,46):type==='tree'?rand(28,40):type==='pillar'?rand(25,34):rand(36,52);if(poi.some(([px,py,pr])=>dist2(x,y,px,py)<(r+pr)*(r+pr)))continue;if(obstacles.some(o=>dist2(x,y,o.x,o.y)<(r+o.r+52)*(r+o.r+52)))continue;obstacles.push({x,y,r,type,rot:rand(0,Math.PI*2),variant:Math.floor(rand(0,3))})}}
...
function update(dt){elapsed+=dt;spawnTimer-=dt;shotTimer-=dt;player.hitFlash=Math.max(0,player.hitFlash-dt);player.moveSlow=Math.max(0,(player.moveSlow||0)-dt);if(player.regen>0)player.hp=Math.min(player.maxHp,player.hp+player.regen*dt);updateWorldInteractions(dt);updateSkills(dt);updateAttackEffects(dt);updateWaveState();if(!midBossSpawned&&elapsed>=MID_BOSS_AT)spawnMidBoss();if(!bossSpawned&&elapsed>=BOSS_SPAWN_AT){bossSpawned=true;spawnBoss()}if(elapsed>=STAGE_DURATION)stageExpired=true;const progress=stageProgress(),bossPhase=bossSpawned&&!finalBossDefeated,waveBoost=currentWave?1:0,baseRate=(.54-progress*.22)*currentMap.spawn-Math.min(.06,(currentStage-1)*.010),rate=currentWave?Math.max(.16,baseRate*.58):(bossPhase?Math.max(.31,baseRate+.04):Math.max(.25,baseRate));if(!stageExpired&&spawnTimer<=0&&enemies.length<MAX_LIVE_ENEMIES){let pack=progress<.18?2:progress<.48?3:4;if(currentWave)pack+=currentWave.type==='swarm'?4:3;if(bossPhase&&!currentWave)pack=Math.max(2,pack-1);pack+=Math.min(2,Math.floor((currentStage-1)/3));for(let n=0;n<pack&&enemies.length<MAX_LIVE_ENEMIES;n++)spawnEnemy();spawnTimer=rate}if(shotTimer<=0){shoot();shotTimer=player.fireRate*(runEvolution ? .88 : 1)}
 ...
 for(const b of bullets){if(b.homing>0&&enemies.length){let t=null,bd=360*360;for(const e of enemies){if(e.hp<=0||!isOnCombatScreen(e,420))continue;const d=dist2(b.x,b.y,e.x,e.y);if(d<bd){bd=d;t=e}}if(t){const sp=Math.hypot(b.vx,b.vy)||1,ta=Math.atan2(t.y-b.y,t.x-b.x),ca=Math.atan2(b.vy,b.vx),na=ca+angleDiff(ta,ca)*Math.min(1,dt*b.homing);b.vx=Math.cos(na)*sp;b.vy=Math.sin(na)*sp}}if(!isIos){b.trail.push({x:b.x,y:b.y});if(b.trail.length>6)b.trail.shift()}b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;if(dist2(b.x,b.y,b.originX,b.originY)>b.maxRange*b.maxRange)b.life=0;if(b.weaponType!=='skill'&&(b.x<camera.x-55||b.x>camera.x+W+55||b.y<camera.y-55||b.y>camera.y+H+55))b.life=0;if(b.life>0&&bulletHitsObstacle(b)){b.life=0;burst(b.x,b.y,3,'#94a8b9')}}bullets=bullets.filter(b=>b.life>0&&b.x>-80&&b.x<WORLD_W+80&&b.y>-80&&b.y<WORLD_H+80);
 for(const b of enemyBullets){if(b.homing>0){const sp=Math.hypot(b.vx,b.vy)||1,ta=Math.atan2(player.y-b.y,player.x-b.x),ca=Math.atan2(b.vy,b.vx),na=ca+angleDiff(ta,ca)*Math.min(1,dt*b.homing);b.vx=Math.cos(na)*sp;b.vy=Math.sin(na)*sp}if(!isIos){b.trail.push({x:b.x,y:b.y});if(b.trail.length>5)b.trail.shift()}b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;if(b.life>0&&bulletHitsObstacle(b))b.life=0;const rr=player.r+b.r;if(b.life>0&&dist2(b.x,b.y,player.x,player.y)<rr*rr){b.life=0;if(damagePlayer(b.damage,b.color,b.slow))return}}enemyBullets=enemyBullets.filter(b=>b.life>0&&b.x>-80&&b.x<WORLD_W+80&&b.y>-80&&b.y<WORLD_H+80);
 ...
 for(const b of bullets){for(const e of enemies){if(e.hp<=0||b.life<=0||b.hitIds.has(e)||Math.abs(b.x-e.x)>54||Math.abs(b.y-e.y)>54)continue;const dx=b.x-e.x,dy=b.y-e.y;if(dx*dx+dy*dy<(b.r+e.r)**2)damageEnemy(e,b)}}for(const e of enemies)if(e.hp<=0&&!e.dead){e.dead=true;killEnemy(e)}enemies=enemies.filter(e=>!e.dead);
...
function drawBackground(){const map=currentMap||MAP_THEMES[0];if(isIos){ctx.fillStyle=map.ground;ctx.fillRect(0,0,W,H);return}const g=ctx.createLinearGradient(0,0,0,H);g.addColorStop(0,map.sky1);g.addColorStop(.52,map.ground);g.addColorStop(1,map.sky2);ctx.fillStyle=g;ctx.fillRect(0,0,W,H);for(const s of stars){const sx=(s.x-camera.x*.12)%WORLD_W,sy=(s.y-camera.y*.12)%WORLD_H;if(sx<-5||sx>W+5||sy<-5||sy>H+5)continue;ctx.globalAlpha=s.a;ctx.fillStyle=map.accent;ctx.beginPath();ctx.arc(sx,sy,s.r,0,Math.PI*2);ctx.fill()}ctx.globalAlpha=1;for(const f of fogWisps){const sx=f.x-camera.x*.18+Math.sin(elapsed*.1+f.phase)*24,sy=f.y-camera.y*.18+Math.cos(elapsed*.08+f.phase)*18;if(sx<-f.r||sx>W+f.r||sy<-f.r||sy>H+f.r)continue;const rg=ctx.createRadialGradient(sx,sy,0,sx,sy,f.r);rg.addColorStop(0,map.id==='ember'?'rgba(180,78,48,.045)':map.id==='frost'?'rgba(120,180,230,.045)':map.id==='crypt'?'rgba(145,92,185,.045)':'rgba(87,136,126,.055)');rg.addColorStop(1,'rgba(20,30,34,0)');ctx.fillStyle=rg;ctx.beginPath();ctx.arc(sx,sy,f.r,0,Math.PI*2);ctx.fill()}}
...
function generateWorld(){selectMapTheme();terrainPatches=Array.from({length:32},(_,i)=>({x:rand(120,WORLD_W-120),y:rand(120,WORLD_H-120),rx:rand(120,330),ry:rand(90,250),rot:rand(0,Math.PI),kind:i%4}));groundDecos=Array.from({length:170},()=>({x:rand(60,WORLD_W-60),y:rand(60,WORLD_H-60),kind:Math.floor(rand(0,5)),rot:rand(0,Math.PI*2),scale:rand(.7,1.45)}));fogWisps=Array.from({length:14},()=>({x:rand(0,WORLD_W),y:rand(0,WORLD_H),r:rand(100,240),speed:rand(4,12),phase:rand(0,Math.PI*2)}));mapPaths=[];for(let p=0;p<2;p++){const horizontal=p!==1,pts=[];if(horizontal){pts.push({x:80,y:rand(WORLD_H*.18,WORLD_H*.82)});pts.push({x:WORLD_W*.32,y:rand(WORLD_H*.15,WORLD_H*.85)});pts.push({x:WORLD_W*.68,y:rand(WORLD_H*.15,WORLD_H*.85)});pts.push({x:WORLD_W-80,y:rand(WORLD_H*.18,WORLD_H*.82)})}else{pts.push({x:rand(WORLD_W*.2,WORLD_W*.8),y:80});pts.push({x:rand(WORLD_W*.15,WORLD_W*.85),y:WORLD_H*.34});pts.push({x:rand(WORLD_W*.15,WORLD_W*.85),y:WORLD_H*.68});pts.push({x:rand(WORLD_W*.2,WORLD_W*.8),y:WORLD_H-80})}mapPaths.push(pts)}mapLandmarks=[];const lmKinds=currentMap.id==='forest'?['shrine','tree','crystal','gate']:currentMap.id==='frost'?['crystal','obelisk','gate','shrine']:currentMap.id==='ember'?['forge','obelisk','crystal','gate']:['obelisk','shrine','gate','crystal'];for(let i=0;i<8;i++){const p=safeWorldPoint(360);mapLandmarks.push({x:p.x,y:p.y,kind:pick(lmKinds),r:rand(32,55),phase:rand(0,Math.PI*2)})}chests=[];for(let i=0;i<8;i++){const p=safeCratePoint(260),reinforced=i>=6,maxHp=reinforced?70:44;chests.push({x:p.x,y:p.y,r:reinforced?18:16,hp:maxHp,maxHp,broken:false,elite:reinforced,tier:reinforced?1:0,hitFlash:0})}springs=[];obstacles=[];const poi=[...chests.map(o=>[o.x,o.y,90]),...mapLandmarks.map(o=>[o.x,o.y,o.r+70]),[WORLD_W/2,WORLD_H/2,330]],types=currentMap.id==='frost'?['rock','pillar','pillar','ruin','ruin','tree']:currentMap.id==='ember'?['rock','rock','rock','ruin','pillar','tree']:currentMap.id==='crypt'?['pillar','ruin','ruin','tree','rock','pillar']:['rock','rock','tree','tree','pillar','ruin'];for(let n=0;n<130&&obstacles.length<42;n++){const x=rand(110,WORLD_W-110),y=rand(110,WORLD_H-110),type=pick(types),r=type==='rock'?rand(28,46):type==='tree'?rand(28,40):type==='pillar'?rand(25,34):rand(36,52);if(poi.some(([px,py,pr])=>dist2(x,y,px,py)<(r+pr)*(r+pr)))continue;if(obstacles.some(o=>dist2(x,y,o.x,o.y)<(r+o.r+52)*(r+o.r+52)))continue;obstacles.push({x,y,r,type,rot:rand(0,Math.PI*2),variant:Math.floor(rand(0,3))})}}
function resetWorldInteractions(){for(const c of chests){c.broken=false;c.hp=c.maxHp;c.hitFlash=0}springs=[]}
function updateWorldInteractions(dt){for(const c of chests)c.hitFlash=Math.max(0,(c.hitFlash||0)-dt);if(zoneBadge.dataset.map!==currentMap.id){zoneBadge.dataset.map=currentMap.id;zoneBadge.classList.remove('danger');zoneBadge.textContent=`${currentMap.icon} ${currentMap.name} · ${currentMap.tag}`;enemyGuide.textContent=MAP_GUIDES[currentMap.roster]||'👹 다양한 적 패턴'}}
...
function v43MissionPoint(){for(let n=0;n<100;n++){const a=rand(0,Math.PI*2),r=rand(480,1280),x=WORLD_W/2+Math.cos(a)*r,y=WORLD_H/2+Math.sin(a)*r;if(x<120||x>WORLD_W-120||y<120||y>WORLD_H-120)continue;if(obstacles.some(o=>dist2(x,y,o.x,o.y)<(o.r+80)*(o.r+80)))continue;if(chests.some(c=>dist2(x,y,c.x,c.y)<100*100))continue;return{x,y}}return{x:WORLD_W/2+520,y:WORLD_H/2}}
...
function v440SeedWorldEnemies(){
  if(!player)return;['basic','fast','tank','ranged','charger'].forEach(t=>seenEnemyTypes.add(t));const target=24+Math.min(10,currentStage);
...
function v440MaintainWorldPopulation(dt){
  if(!player||stageExpired)return;v440PopulationTimer-=dt;if(v440PopulationTimer>0)return;v440PopulationTimer=1.45;
...
/* ===== v4.5.7 SMOOTH MOBILE PERFORMANCE GUARDS ===== */
...
/* ===== END v4.5.7 SMOOTH MOBILE PERFORMANCE GUARDS ===== */
